...
def pad_or_truncate(seq, target_length=desired_sequences_length):
    """
...
def load_gesture_data(gesture_folder, target_length=desired_sequences_length):
    """
...
def load_all_gestures(base_path, target_length=desired_sequences_length):
    """
...
def load_predict_data(predict_path, target_length=desired_sequences_length):
    """
...
def build_model(input_shape, hidden_dim, output_dim, dropout_rate=0.2, weight_decay=0.0):
    """
    Build a CNN model that accepts a desired_sequences_length x 130 input (with 1 channel).
    The architecture uses only TFLite built-in operators.
    """
    inputs = tf.keras.Input(shape=input_shape)  # Expected shape: (desired_sequences_length, 130, 1)
...
def main():
    ...
    # Each file is expected to be a desired_sequences_length-row x 130-column matrix.
    rows = desired_sequences_length        # number of rows (or “time steps”)
    cols = 130        # number of columns (features)
    channels = 1
    input_shape = (rows, cols, channels)  # (20, 130, 1)
    ...
    if select == '1':
        ...
        train_data = np.expand_dims(train_data, -1)  # shape becomes (num_samples, 20, 130, 1)
        ...
        predict_data = np.expand_dims(predict_data, -1)  # shape (num_samples, 20, 130, 1)
