...
import joblib  # Added for saving and loading the scaler
...
def main():
    ...
    if select == '1':
        ...
        train_data, scaler = normalize_sequences(train_data, fit_scaler=True)
        ...
        test_data, _ = normalize_sequences(test_data, scaler=scaler, fit_scaler=False)

        # Convert data and labels to tensors
        # Note: We won't pad sequences here; we'll handle them in the data loader
        ...
        # Adjust input_size based on the data
        predict_data, filenames = load_predict_data(predict_path)
        if len(predict_data) == 0:
            print("No data found in the predict path.")
            return

        input_size = predict_data[0].shape[1]  # Number of features per time step
        ...
        # Normalize the predict data using the same scaler
        predict_data, _ = normalize_sequences(predict_data, scaler=scaler, fit_scaler=False)
