...
def main():
    ...
    if select == '1':
        ...
        # Scale the data since it is no longer binary
        def scale_data(data_list, min_value=0, max_value=2):
            scaled_data_list = []
            for seq in data_list:
                scaled_seq = (seq - min_value) / (max_value - min_value)
                scaled_data_list.append(scaled_seq)
            return scaled_data_list

        train_data = scale_data(train_data, min_value=0, max_value=2)
        test_data = scale_data(test_data, min_value=0, max_value=2)
                                                ...
                                                for fold, (train_idx, val_idx) in enumerate(kfold.split(train_dataset)):
                                                    ...
                                                    def collate_fn(batch):
                                                        ...
                                                        sequences, labels, lengths = zip(
                                                            *[(torch.tensor(seq, dtype=torch.float32), label, len(seq))
                                                              for seq, label, length in batch])
        ...
        # Scale the predict data (Apply the same scaling as during training)
        def scale_data(data_list, min_value=0, max_value=2):
            scaled_data_list = []
            for seq in data_list:
                scaled_seq = (seq - min_value) / (max_value - min_value)
        ...
        predict_data = scale_data(predict_data, min_value=0, max_value=2)
        ...
        def collate_fn_predict(batch):
            batch.sort(key=lambda x: len(x[0]), reverse=True)
            sequences, lengths, filenames_batch = zip(
                *[(torch.tensor(seq, dtype=torch.float32), len(seq), filename)
                  for seq, length, filename in batch])
